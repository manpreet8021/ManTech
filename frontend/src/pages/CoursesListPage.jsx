...
export default function CoursesListPage() {
  ...
  const handleCreate = ({ title, description }) => {
    setCourses((prev) => [
      ...prev,
      { course_id: Math.max(0, ...prev.map((c) => c.course_id)) + 1, title, description },
    ])
    setModalOpen(false)
  }
  ...
  return (
    <div>
      ...
      <div className="mt-8 grid grid-cols-1 gap-6 sm:grid-cols-2 lg:grid-cols-3">
        {courses.map((course) => {
          const videoCount = mockVideos.filter((v) => v.course_id === course.course_id).length
      ...
      <CreateCourseModal open={modalOpen} onClose={() => setModalOpen(false)} onCreate={handleCreate} />
    </div>
  )
}
